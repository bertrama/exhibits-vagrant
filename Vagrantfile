...
Vagrant.configure(2) do |config|
  ...
  # using a specific IP.
  config.vm.network "private_network", ip: "10.255.21.11"
  ...
  config.vm.provision "trigger", good_exit: [0, 128] do |trigger|
    trigger.fire do
      unless File.exist?('sites/exhibits')
        run "mkdir -p sites"
        run "git clone https://github.com/mlibrary/online-exhibits sites/exhibits"
      end
    end
  end
